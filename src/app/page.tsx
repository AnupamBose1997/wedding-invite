...
import { getInviteConfig, filterCeremoniesForInvite, getNavigationItemsForInvite } from '@/lib/inviteFilter'
...
export default async function Home({ searchParams }: { searchParams: { invite?: string } }) {
  const data = await getWeddingData()
  
  // Get invite configuration based on URL parameter
  const inviteConfig = getInviteConfig(searchParams.invite)
  
  // Filter ceremonies based on invite type
  const filteredCeremonies = filterCeremoniesForInvite(data.ceremonies, inviteConfig.showCeremonies)
  
  // Choose which RSVP data to use based on invite type
  const rsvpData = inviteConfig.rsvpFormType === 'reception' && data.receptionRSVP 
    ? data.receptionRSVP 
    : data.rsvp
  ...
  return (
    <main>
      ...
      <Navigation 
        titleFont={data.hero.titleFont} 
        logo={data.siteSettings.logo}
        navItems={getNavigationItemsForInvite(inviteConfig)}
      />
      
      {inviteConfig.showHero && (
        <section id="home">
          <Hero 
            hero={{
              ...data.hero,
              subtitle: inviteConfig.customSubtitle || data.hero.subtitle
            }} 
          />
        </section>
      )}
      ...
      {inviteConfig.showStory && (
        <section id="story">
          <Story story={data.story} />
        </section>
      )}
      
      {filteredCeremonies && inviteConfig.showCeremonies.length > 0 && (
        <section id="venue">
          <Venue 
            venue={data.venue} 
            ceremonies={filteredCeremonies} 
            timezone={data.siteSettings.timezone} 
          />
        </section>
      )}
      
      {inviteConfig.showGallery && data.gallery && (
        <section id="gallery">
          <Gallery gallery={data.gallery} />
        </section>
      )}
      
      {inviteConfig.showRSVP && (
        <section id="rsvp">
          <RSVP 
            rsvp={rsvpData} 
            timezone={data.siteSettings.timezone}
            inviteType={inviteConfig.type}
          />
        </section>
      )}
